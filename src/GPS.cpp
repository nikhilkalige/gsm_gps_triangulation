...
#if 0
void Adafruit_GPS::sendCommand(char cmd, char time)
{
    char str[24];
    char ch[7];
    char i, chk;
    strcpy_P(str, (char *)psrf_cmd);
    str[cmd_pos] = cmd;
    str[period_pos] = time | 0x30;
    chk = 0;
    for (i = 1; i < 20; i++)
    {
        chk ^= str[i];
    ...
    itoa(chk, ch, 16);
    str[chksum_pos] = ch[0];
    str[chksum_pos + 1] = ch[1];
    gpsSwSerial->println(str);
}
#else
void Adafruit_GPS::sendCommand(char *string)
{
    char c;
